...
def reporte_distribucion_trastornos_pref_sexual(filename):
    ...
    for row in range(2, ws.max_row + 1):
        try:
            pref_sexual = ws.cell(
                row=row, column=4
            ).value  # pref_sexual de origen (Columna 4)
    ...
    for row_num, (pref_sexual, cantidad) in enumerate(
        pref_sexuals_registradas.items(), 2
    ):
        fila = [pref_sexual, cantidad] + [
            conteo_trastornos_por_pref_sexual[pref_sexual][t]
    ...
    data = Reference(
        ws_reporte, min_col=2, min_row=1, max_row=len(pref_sexuals_registradas) + 1
    )
    categories = Reference(
        ws_reporte, min_col=1, min_row=2, max_row=len(pref_sexuals_registradas) + 1
    )
    ...
    # Encabezados
    ws_reporte.append(
        ["preferencia sexual", "Cantidad de Personas"]
    ...
    for row_num, (pref_sexual, cantidad) in enumerate(
        pref_sexuals_registradas.items(), ws_reporte.max_row + 1
    ):
        ws_reporte.append(
            [pref_sexual, cantidad]
            + [
                (
                    conteo_trastornos_por_pref_sexual[pref_sexual][t] / cantidad
                    if cantidad != 0
                    else 0
                )
                for t in encabezados_trastornos
    ...
    crear_tabla(
        ...
        table_name="Tabla_pref_sexual_origen",
    )
