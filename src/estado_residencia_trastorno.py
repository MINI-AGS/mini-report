...
def reporte_distribucion_estado_residencia_trastorno(filename):
    ...
    for row in range(2, ws.max_row + 1):
        try:
            estado = ws.cell(
                row=row, column=6
            ).value  # Estado de residencia (Columna 5)
    ...
    # Encabezados
    ws_reporte.append(
        ["Estado de Residencia", "Cantidad de Personas"] + encabezados_trastornos
    )
    ...
    # Encabezados
    ws_reporte.append(
        ["Estado de Residencia", "Cantidad de Personas"]
    ...
    crear_tabla(
        ws_reporte, type_table="TableStyleMedium2", table_name="Tabla_Estado_Residencia"
    )
    ...
    radar_chart.title = (
        "Distribución de Promedio de Trastornos por Estado de residencia"
    )
