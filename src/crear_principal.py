...
def crear_tabla_principal(path="false_data/jtest_data.json"):
    ...
    for row_num, fila in enumerate(lineas, 2):
        ...
        cantidad_si = sum(1 for i in indice_trastornos if i - 1 < len(fila) and fila[i - 1].strip() != "")
    ...
    try:
        wb.save(output_path)
        wb.close()
        print(f"Reporte principal guardada en: '{output_path}'")
    except Exception as e:
